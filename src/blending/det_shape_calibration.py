...
def _direct_calibrate(g_cond, R_cond, mask_cond,
    ...
    def _compute_mc(g_list, R_list, m_list):
        """Compute m, c from 4 per-condition means.

        g_means[i] shape (2,), R_means[i] shape (2, 2).
        Condition order: 0=g1+,1=g1-,2=g2+,3=g2-.
        """
        # Per-condition means (masked)
        g_means = np.zeros((4, 2), dtype=np.float64)
        R_means = np.zeros((4, 2, 2), dtype=np.float64)
        for i in range(4):
            gi = g_list[i]
            Ri = R_list[i]
        ...
        m1 = (shear_1p - shear_1m) / denom1 / shear_value - 1.0 if denom1 != 0 else np.nan
        m2 = (shear_2p - shear_2m) / denom2 / shear_value - 1.0 if denom2 != 0 else np.nan
        c1 = (shear_1p + shear_1m) / denom1 if denom1 != 0 else np.nan
        c2 = (shear_2p + shear_2m) / denom2 if denom2 != 0 else np.nan
...
def get_det_calibration_biases(
    ...
    # ---- shear tasks (matching cal_biases.py convention) ----
    # i=0: g1+1 (shear_mode=1, shear_comp='g1')
    # i=1: g1-1 (shear_mode=0, shear_comp='g1')
    # i=2: g2+1 (shear_mode=1, shear_comp='g2')
    # i=3: g2-1 (shear_mode=0, shear_comp='g2')
    shear_tasks = [
        (1, "g1"),
        (0, "g1"),
        (1, "g2"),
    ...
    for mode in modes:
        ...
        # batch_data[index] = list of 4 result tuples (or None placeholders)
        batch_data = {}
        n_missing = 0
        n_loaded = 0
        ...
        with mp.Pool(processes=workers) as pool:
            for result in tqdm(
                ...
                if index not in batch_data:
                    batch_data[index] = [None, None, None, None]
                batch_data[index][i] = (
        ...
        # In real observations, each shear condition is an independent sample of
        # different galaxies.  No alignment across conditions is needed.
        # We collect all galaxies per condition, then truncate to a common N
        # before stacking into the (N, 4, ...) arrays.
        cond_data = {0: [], 1: [], 2: [], 3: []}
        ...
        # Concatenate per condition
        g_concat = []   # list of 4 arrays (N_i, 2)
        R_concat = []   # list of 4 arrays (N_i, 2, 2)
        w_concat = []   # list of 4 arrays (N_i,)
        dw_concat = []  # list of 4 arrays (N_i, 2)
        m00_concat = [] # list of 4 arrays (N_i,)
        dm00_concat = []# list of 4 arrays (N_i, 2)
        ...
        # Keep per-condition arrays as-is (no truncation, no stacking).
        # Different conditions may have different N_i — that's fine:
        # we compute per-condition means then plug into the bias formula.
        N_per_cond = [g.shape[0] for g in g_concat]
        print(f"[assemble] Per-condition N: {N_per_cond}")

        if any(n == 0 for n in N_per_cond):
            print("[assemble] ERROR: one condition has 0 galaxies.")
            all_results[mode] = (np.full(2, np.nan), np.full(2, np.nan),
                                 np.full(2, np.nan), np.full(2, np.nan))
            continue
        ...
        # ---- Log to CSV ----
        _append_csv(log_csv, noise_level, fname, mode, mag_cut, cal_result)
...
def _append_csv(log_csv, noise_level, fname, mode, mag_cut, cal_result):
