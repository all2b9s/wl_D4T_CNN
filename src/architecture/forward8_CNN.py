...
class Forward8_ShapeletHead_CNN(Forward8_fixW_CNN):
    """
    Based on Forward8_fixW_CNN, reads out features using shapelet-like moments.
    Only changes the head, not the trunk / D4 structure / normalization.
    """
    ...
    def __init__(
        ...
        """
        use_gap_feature: if True, append the original GAP vector alongside the shapelet moment features.
        """
        super().__init__(
            in_dim=in_dim,
        ...
        self._coord_cache = {}  # cache (Hf,Wf,device,dtype) -> (xx,yy)

        C = base_channels
        # compute (M00, M22c, M22s) three moments per channel -> 3C dims
        in_dim_head = 3 * C
        if self.use_gap_feature:
            # append the original GAP (C-dim) feature if requested
            in_dim_head += C

        # rebuild head_e1, head_e2 to accept the new input dimension
        self.head_e1 = BiasFreeMLP(in_dim_head, hidden=head_hidden)
        self.head_e2 = BiasFreeMLP(in_dim_head, hidden=head_hidden)

    # ------------------------------------------------------------
    # internal utilities: cached coordinate grids and shapelet-like moments
    # ------------------------------------------------------------
    def _get_coord_grid(self, hw, device, dtype):
        """
        Return coordinate grids xx,yy normalized to [-1,1], shape [1,1,H,W],
        used to build (x^2-y^2), 2xy.
        """
    ...
    def _shapelet_moments(self, F, w_f, xx, yy, eps=1e-8):
        """
        Compute shapelet-like moments on the feature map.
        F:   [B,C,Hf,Wf]
        w_f: [1,1,Hf,Wf]  Gaussian weights
        xx,yy: [1,1,Hf,Wf] matching coordinate grids

        Returns:
          moms: [B,C,3]  (M00, M22c, M22s) per channel
        """
        # normalize the weights so that sum_w ~ 1 (avoid numerical offset)
        w_norm = w_f / (w_f.sum(dim=(-2, -1), keepdim=True) + eps)  # [1,1,Hf,Wf]

        # 0th-order moment: like M00
        M00 = (F * w_norm).sum(dim=(-2, -1))  # [B,C]

        # 2nd-order moment kernels: Qc = x^2 - y^2, Qs = 2xy
        Qc = xx * xx - yy * yy
        Qs = 2.0 * xx * yy
    ...
    # ------------------------------------------------------------
    # Forward: replicate the parent forward flow, only changing the head part
    # ------------------------------------------------------------
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # --- D4 orbit and inverse aggregation (fully reuses parent logic) ---
        xs = self._d4_orbit_stack(x)  # [8,B,C,H,W]
        B = x.size(0)
        x8 = xs.view(-1, xs.size(2), xs.size(3), xs.size(4))  # [8B,C,H,W]
        ...
        # inverse back to canonical frame
        feats = self._d4_inverse_stack(f8)  # [8,B,C,Hf,Wf]

        # --- sign-weighted mean for e1/e2 (same as parent) ---
        s1 = self.signs_e1.view(8, 1, 1, 1, 1).to(feats)
        s2 = self.signs_e2.view(8, 1, 1, 1, 1).to(feats)
        ...
        # --- coordinate grid (generated once from Hf,Wf and cached) ---
        ...
        # --- compute shapelet-like moments on the feature maps ---
        ...
        if self.use_gap_feature:
            # optionally append the original weighted GAP features (another statistic of M00)
...
class Forward8_attnW_CNN(Forward8_base):
    ...
    def forward(self, x):
        ...
        self.f_mean_e1 = (feats * s1).mean(dim=0)  # [B,C,Hf,Wf]
        self.f_mean_e2 = (feats * s2).mean(dim=0)  # [B,C,Hf,Wf]
        f_inv = feats.mean(dim=0)                       # [B,C,Hf,Wf]
