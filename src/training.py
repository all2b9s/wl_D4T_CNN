...
class OptunaTrainer:
    ...
    def _objective(self, trial: optuna.trial.Trial) -> float:
        ...
        best_val = float("inf")
        trial_ckpt = os.path.join(self.save_dir, f"trial{trial.number}_best.pt")  # NEW
        ...
        for ep in range(1, self.epochs + 1):
            ...
            if va < best_val:
                ...
                trial.set_user_attr("ckpt_path", trial_ckpt)
...
def main():
    ...
    tuner = OptunaTrainer(
        ...
        device=device,
        seed=args.seed,
        save_dir=args.storage+"D4T_xlens_noiseless/",
    )
    tuner.run(
        n_trials=args.n_trials,
        study_name=args.study_name,
        storage=args.storage+"optuna_studies/D4T_xlens_noiseless.db",
